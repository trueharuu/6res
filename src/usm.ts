...
export function unhashBoard(boardHash: number): Board {
  const board: Board = [];
  while (boardHash > 0) {
    let rowHash = boardHash % 16;
    boardHash = Math.floor(boardHash / 16);
    const row: number[] = [];
    for (let squareNum = 0; squareNum < 4; squareNum++) {
      row.push(rowHash % 2);
      rowHash = Math.floor(rowHash / 2);
    }
    board.push(row);
  }
  return board;
...
// Displays a board
export function displayBoard(boardHash: number): void {
...
export function displayBoards(boardHashList: number[]): void {
...
// Assume 100g.
interface Result { board: number, finesse: Array<string> }
export function getNextBoards(boardHash: number, piece: string): number[] {
  // Obtain board
  const board = unhashBoard(boardHash);
  ...
  const queue: [number, number, number][] = [[y, 1, 0]];
  ...
  while (queue.length > 0) {
    ...
    if (!visited.has(currentKey)) {
      ...
      for (const xMove of [-1, 1]) {
        ...
        if (newYOffset <= 0) {
          queue.push([y + newYOffset, x + xMove, rotation]);
        }
      }
      ...
      for (const rotationMove of [1, 2, 3]) {
        ...
        for (const [kickOffsetY, kickOffsetX] of KICKS[piece][rotation][
          ...
          if (good) {
            ...
            queue.push([newYPosition, newXPosition, newRotation]);
            break;
          }
        }
      }
  ...
  for (const visitedState of visited) {
    const [y, x, rotation] = visitedState.split(",").map(Number);
    let newHash = boardHash;

    for (const [offsetY, offsetX] of PIECES[piece][rotation]) {
      newHash += 2 ** (4 * (y + offsetY) + x + offsetX);
    }
    ...
    boards.add(hashBoard(clearedBoard));
  }
